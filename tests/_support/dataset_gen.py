"""Deterministic dataset generator for the DERSIS regression suite.

Promoted (minus its sys.path bootstrap) from the stress-test audit asset
``stress-test/tests/_fixtures/dataset_gen.py`` so the suite is self-contained.
Builds state dicts matching ``scheduler_app.core.models.new_state()`` /
``new_class()`` shapes at parameterized scale and constraint density. Fully
deterministic via *seed*. No Qt, no storage side effects.

    from _support.dataset_gen import make_state, make_preset
