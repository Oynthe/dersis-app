"""INDEPENDENT correctness oracle for DERSIS schedules.

Promoted from the stress-test audit asset ``stress-test/tests/schedule_oracle.py``
(sandbox bootstrap and CLI driver removed; ``check_schedule`` itself is
byte-identical -- it is the correctness spine and must not drift).
...
The only production helpers used are pure structural readers (total_duration,
target activity for non-joint blocks). Availability and capacity are
re-implemented locally and defensively (missing keys tolerated) so the oracle
...
# --------------------------------------------------------------------------
# Reusable driver: run the production workflow on a preset and audit both the
# raw optimizer output and the state that actually gets committed.
# --------------------------------------------------------------------------
def audit_preset(preset_name, seed=42, use_cpsat=False, weights=None):
    """Reschedule *preset_name* through the production workflow and audit it.

    Returns a dict with:
        raw      -- check_schedule() over ``result.placed`` (what the optimizer
                    actually proposed, before apply_reschedule drops losers)
        applied  -- check_schedule() over the committed state
        rejected -- number of placements apply_reschedule refused
        result   -- the raw workflow result object
        state    -- the (now mutated) state dict
    """
    from _support.dataset_gen import make_preset
    from scheduler_app.core.workflow import SchedulingWorkflow

    state = make_preset(preset_name, seed=seed)
    w = dict(weights or {})

    locked_baseline = {}
    for c in state["classes"]:
        if c.get("protection") == "locked" and not c.get("pinned") and c.get("placed"):
            locked_baseline[c["class_uid"]] = (
                c["placed_day"], c["placed_time"], c["placed_classroom"])

    wf = SchedulingWorkflow(state, lambda: dict(w))
    result = wf.reschedule(w, use_cpsat=use_cpsat)

    raw = check_schedule(state, placements=result.placed)
    rejected = wf.apply_reschedule(result)
    applied = check_schedule(state, locked_baseline=locked_baseline)
...
def hard_violation_count(audit):
    """Total hard-invariant violations in a check_schedule() result.

    Capacity is excluded: DERSIS treats room capacity as a soft preference
    (``get_room_candidates(apply_capacity=False)`` is a supported path), so it
    is reported by the oracle but not counted as a hard breach here.
    """
    return sum(n for cat, n in audit["counts"].items() if cat in HARD_CATEGORIES)


def format_violations(audit, limit=10):
    """Human-readable violation digest for assertion messages."""
    lines = [f"{cat}={n}" for cat, n in sorted(audit["counts"].items())]
    head = ", ".join(lines) or "none"
    samples = [f"  [{v['category']}] {v['cls']}: {v['detail']}"
               for v in audit["violations"][:limit]]
    return "\n".join([head] + samples)
