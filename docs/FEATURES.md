...
- **Email-based**: Composes the report locally (app version, OS, severity, steps, optional traceback) and opens the user's default email client via `mailto:emre.uygun.elt@gmail.com`; falls back to copying the text to the clipboard if no mail client is configured
