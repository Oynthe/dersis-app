...
## ⚠ This file cannot currently answer "what is still open"

Established while trying to use it that way in Phase 8, and recorded so the next
reader does not repeat the attempt:

* The **status lives in the last column of the summary tables**, appended after
  `OBSERVED` as `-> **Fixed** (Phase N)` or similar. It is *not* a separate
  column, so it cannot be parsed positionally.
* **Only 28 of the 94 findings have a detail block**, and therefore only 28 have
  a `**Status**` line. All 28 read closed — which makes the detail blocks look
  authoritative and they are not, because the other 66 findings simply have no
  block.
* Many summary rows still read a bare `OBSERVED` for findings closed in Phases
  0–1. A row with no `Fixed`/`Closed` annotation means *nobody has updated this
  row*, **not** that the finding is open.

So the real open/closed state has been tracked in [`PROGRESS.md`](PROGRESS.md),
in the suite's `xfail` pins, and in the phase handoffs — not here. Treat this
file as the canonical **description** of each finding and the canonical ID
namespace; treat the handoff for the current phase as the canonical **backlog**.

Making this machine-checkable (one status token per row, plus a test that parses
it) is worth doing and was deliberately **not** attempted in Phase 8: a
94-row restructure at the end of a long session is a good way to introduce the
very false claims the phase existed to remove.
...
| ST-FUNC-007 | 🟡 Medium | High | Legacy plain-JSON ASCII saves cannot be loaded (mis-detected as Fernet tokens); only non-ASCII JSON loads — inverse of intent | OBSERVED -> **Fixed** (Phase 8). `_is_fernet_token` asked "do the first 80 bytes decode as ASCII", which is true of every plain-JSON file ever written in ASCII. It now asks the positive question — a Fernet token is urlsafe-base64 of `0x80` + an 8-byte timestamp whose top four bytes stay zero until 2106, so it literally begins `gAAAAA` (measured: 20 of 20 generated tokens began `gAAAAABq`). Bracketed by a **pair** of tests so neither a constant-True nor a constant-False predicate can pass both |
| ST-FUNC-008 | 🟡 Medium | High | Ctrl+C on the Dashboard tab raises `IndexError` | OBSERVED |
| ST-FUNC-009 | 🟡 Medium | High | `required_room_type` is advertised in template + import schema but never consumed by the importer | OBSERVED -> **Fixed** (Phase 8), with two caveats worth carrying. Resolved to room *names* at import into the existing `required_classrooms`, matched against the room's own `room_type` cell **from the same workbook** (never the room's name: that passes in tr by accident, where the lab room is "Laboratuvar A" and its type "Laboratuvar", and fails in nl/az/af/da/id/pl). Intersects with `allowed_rooms`, may only **narrow**, and never writes an empty list — empty means "any room", the exact inversion of the column. The register's suggested wiring into `InfeasibilityAnalyzer` **does not work**: `required_room_missing` is unreachable when `required_classrooms` is empty. CAVEATS: still open through **joint sessions** (`_resolve_joint_groups` merges only `targets`, never `required_classrooms`), and the resolved names are a snapshot `reconcile_placements` never repairs — both in HANDOFF-PHASE9 §B |
...
| ST-FUNC-012 | 🟢 Low | High | No dedup of duplicate `class_code` / classroom / lecturer names — silently accepted | OBSERVED -> **Fixed** (Phase 7 for the report, Phase 8 for the rule). Phase 7 reported duplicates case-insensitively; Phase 8 replaced `str.casefold()` with the shared `i18n/text_fold.py::fold_text` so the dotted/dotless I no longer splits one teacher into two, and gave the refusal its own message naming both spellings instead of the generic `errors.duplicate_values`. The importer and the class form now share **one definition**, so they cannot diverge by construction rather than by discipline. OPEN: the two surfaces still apply opposite *policies* — the importer refuses, the class form silently reassigns (HANDOFF-PHASE9 §B3) |
...
| ST-DATA-002 | 🟠 High | High | Corrupt/truncated feedback log is silently swallowed to `[]`, then the next append overwrites it — history destroyed | OBSERVED -> **Partly closed** (Phase 7). The append half is **already fixed** and the pin was describing deleted code: ST-PERF-005 rewrote `append_encrypted_entry` into an O(1) append, and the damaged bytes survive verbatim as a prefix (measured 300 -> 398 bytes, prefix intact). The swallow half is real and understated -- `_read_log_records` is deliberately tolerant of a torn tail, and `load_encrypted_lines`' blanket `except Exception: return []` discards the healthy records too (0 of 3 recovered), silently disabling preference learning forever -> **Closed** (Phase 8). The swallow half is fixed and the user is told. `_walk_log_frames` is now the single framing rule shared by the reader, `log_entry_count` and `load_encrypted_lines_since`, so damage costs the damaged records only; `PreferenceLearner` advances its cursor past unreadable bytes instead of re-reading them forever, and surfaces the loss through the existing `_report_settings_problem` channel. Making it *raise*, as the pin asked, was built and MEASURED to be worse: the append path swallows the exception, so a silent learning outage became a silent write outage with the quarantine lost too. **Phase 8's own first fix reintroduced this finding** — a damaged container magic was stepped over with `lost == 0`, where the reader it replaced reported `lost == 1` — caught by the adversarial stage and fixed. Invariants now measured exhaustively (7 200 single-bit flips + 895 truncations): 0 fabrications, 0 silent losses, 0 false alarms. OPEN: three blind spots in *when* the report fires, HANDOFF-PHASE9 §B7-B9 |
...
| ST-DATA-009 | 🟡 Medium | High | Dragging one class from the Unplaced panel destroys an unrelated undo-history entry; undo model covers only `state['classes']` | OBSERVED -> **Fixed** (Phase 8). Both halves. The undo model became full-state in Phase 6 (ST-ARCH-012); the *destroys an unrelated entry* half survived until Phase 8 and was **worse than this row says** — it was not a sidebar quirk but `_execute_drop`'s unconditional `if self._undo_stack: self._undo_stack.pop()`, which a sidebar drag reached having pushed nothing, so it consumed whatever action was on top. Depth was unchanged, which is why six phases missed it. **Phase 8 rediscovered this defect independently and did not connect it to this row** — its own handoff listed only the "one Ctrl+Z after a drag unplaces the lesson" half, so the recon agent reported the sidebar case as "not pinned by anything". It was, here. Now guarded by `tests/test_drag_and_drop.py::test_a_sidebar_drag_does_not_eat_the_previous_undo_entry` |
| ST-DATA-010 | 🟡 Medium | High | Multi-select drag from the grid moves only the primary class | OBSERVED -> **Open, and now pinned as-is** (Phase 8). Confirmed exactly as written: `_start_drag_gfx` puts the whole selection in `_dragging_classes` but backs up and unplaces only the primary, so `_execute_drop`'s `all(not c.get("placed"))` guard is False and the single-lesson path runs. Deliberately **not** changed — choosing between "move all N", "refuse" and "tell the user only one moved" is a product decision, not a bug fix, and changing it silently would smuggle a behaviour change into a defect repair. `tests/test_drag_and_drop.py::test_a_multi_select_drag_is_one_undo_for_what_actually_moved` records what the app does today (and that one Ctrl+Z restores it), so whoever makes the product call has a test to change rather than a behaviour to rediscover |
...
| ST-ARCH-012 | 🟡 Medium | High | Undo model covers only `state['classes']`; setup/availability edits are irreversible and can desync restored classes | OBSERVED -> **Fixed** (Phase 6). Snapshots are full-state and Setup is undoable — the fix Phase 4 built and withdrew as "a data-corruption bug wearing a safety label". The cost premise is stale: full-state measures **+1.6-2.9 % time, +2.6-3.8 % memory** over the whole 50-entry stack, and Phase 2 removed the per-refresh encrypted write it was said to stack on. The trap is aliasing, not cost: rebinding `state_data` leaves `SchedulingWorkflow` on the pre-undo dict, so the restore is in place **Phase 8 closed the drag half**: `_execute_drop` re-labels the snapshot already on the stack instead of popping and pushing a fresh one taken after `mark_unplaced`, so one Ctrl+Z after a drag puts the lesson back where the drag found it. Guarded by an explicit `_drag_undo_pushed` flag rather than by `if self._undo_stack`, because the bare `if` is what let a sidebar drag consume an unrelated action's snapshot (ST-DATA-009). Both of the fixes the Phase 8 handoff proposed were built and BOTH failed that case. OPEN: the pre-emptive snapshot is still pushed before the gesture is known to commit, so a cancelled drag destroys the redo stack and can evict the oldest entry at the 50-entry cap — HANDOFF-PHASE9 §B1-B2, one root cause, one cure. |
