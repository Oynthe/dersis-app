...
- All reports are composed **locally** and handed to the user's default email client via a `mailto:emre.uygun.elt@gmail.com` link (subject "DERSİS Bug Report") through `QDesktopServices.openUrl`. If no mail client is configured, the report text is copied to the clipboard and a dialog shows the address. Nothing is transmitted over the network (no `bug-report.php` POST).
