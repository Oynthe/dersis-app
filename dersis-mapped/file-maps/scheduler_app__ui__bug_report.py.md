...
| Symbol | Purpose |
|--------|---------|
| `BUG_REPORT_EMAIL` | `"emre.uygun.elt@gmail.com"` — the destination address. |
...
| `CrashReportDialog` | Auto-shown by `_global_exception_handler`. Pre-fills exception name + message + traceback + log path; collapsible traceback + optional user note; "Report This Crash" composes the body and calls `_open_mailto`. |
...
- Uncaught exception → `scheduler_gui._global_exception_handler` → `CrashReportDialog`.
...
- `scheduler_gui._global_exception_handler` instantiates `CrashReportDialog`.
...
## 12. Mini Summary
Manual bug report + automatic crash report dialogs that compose a `mailto:emre.uygun.elt@gmail.com` message and hand it to the user's email client (clipboard fallback). No backend, no threads, no network — fully offline.
