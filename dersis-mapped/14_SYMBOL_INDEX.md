...
- `BugReportButton` / `BugReportDialog` / `CrashReportDialog` — `ui/bug_report.py` — Bug + crash report flows. Compose a `mailto:emre.uygun.elt@gmail.com` message via `QDesktopServices` (clipboard fallback); no network submission.
